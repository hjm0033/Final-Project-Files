...
##       and then use MULTIQC to evaluate the quality of the data: https://www.bioinformatics.babraham.ac.uk/projects/multiqc/
...
WD=/scratch/$MyID/DogBrainProjectTemp                          ## Example: WD=/scratch/$MyID/PracticeRNAseq
DD=/scratch/$MyID/DogBrainProjectTemp/SingleEndReads                          ## Example: DD=/scratch/$MyID/PracticeRNAseq/RawData
CD=/scratch/$MyID/DogBrainProjectTemp/CleanData                          ## Example: CD=/scratch/$MyID/PracticeRNAseq/CleanData
...
### Make list of file names to Trim
        ...
        ## sort the list and keep only the unique names and put it into a file named "list_single"
### Must move single end reads into SingleEndReads directory before executing.
## File extension appears to not be getting cut. Will troubleshoot later.
#ls | grep ".fastq" |cut -d "_" -f 1 | sort | uniq > list_single
...
fastqc ${CD}/"$i"_unpaired.fastq --outdir=${WD}/${PCQ}
fastqc ${CD}/"$i"_unpaired.fastq --outdir=${WD}/${PCQ}

done<list_single			# This is the end of the loop

        ########## MULTIQC to assess quality of the cleaned sequence data.
## The output from this analysis is a folder of results and a zipped file of results.
#multiqc ${WD}/${PCQ}

##  Now compress your results files from the Quality Assessment by MULTIQC
